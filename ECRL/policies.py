...
class ParticleGNN(nn.Module):
    """
    particle-based multi-head "masked GNN" layer with output projection
    - self, cross, hybrid の3つのモード
    - 入力/出力形状、および引数は元の ParticleAttention とほぼ同じ
    ...
    def __init__(self, embed_dim, n_head, attn_pdrop=0.0, resid_pdrop=0.0,
        ...
        # GATConvは out_channels x heads = 最終出力次元 となる
        # embed_dim をそのまま出したい場合、 out_channels = embed_dim // n_head
    ...
    def forward(self, x, c=None, mask=None, return_attention=False):
        ...
        x: (B, N, T, C)
        c: (B, M, T, C) or None
        mask: shape は実装者が自由に設計 (None か、(B, ?) など)
              True/1 の箇所を「無効化」するとみなす方針
        return_attention: True にすると GATConv の注意重みを返す

        処理の流れ:
          1) バッチ中のサンプルをすべて「1つの大きなグラフ」にまとめる (ノード特徴の結合＋エッジリスト作成)
          2) GATConv を1回だけ呼び出して一括計算
          3) 結果を分割して元の (B, N(or M or N+M), T, C) に戻す
        """
        device = x.device
        B, N, T, C = x.size()
        # print("x", x.shape)
        ...
        if return_attention:
            raise NotImplementedError("return_attention=True は未実装です")
        ...
        if self.att_type == 'hybrid':
            ...
            key_value_N = N
            key_value_T = T
        M = c.size(1) if c is not None else 0  # c: (B, M, T, C)
        ...
        if self.last_shape is None or self.last_shape != cur_shape:
            ...
            edge_list = []
            for b in range(B):
                offset = b * key_value_N * key_value_T
                edge_list_b = self.comb_indices + offset
                edge_list.append(edge_list_b)
            self.edge_list = torch.cat(edge_list, dim=0).to(device)

        pairs_batched = key_value_input_reshape[:, self.comb_indices, :]  # (B, E, C)
        B_, K, two, C_ = pairs_batched.shape
        # print("pairs_batched", pairs_batched.shape)
        pairs = pairs_batched.view(B_ * K, two * C_)
        # print("pairs", pairs.shape)
        self.last_shape = cur_shape

        edge_weight = self.link_prediction(pairs)# .squeeze(-1)
        # print("edge_weight", edge_weight.shape)
        # print("edge_list", self.edge_list.shape)
        
        # GCNの一括処理のためにノード特徴を Flatten
        key_value_input_flatten = key_value_input_reshape.view(B * key_value_N * key_value_T, C)  # (B * N * T, C)
        if mask is not None:
            mask_paird = mask[:, 0, self.comb_indices]
            # print("mask_paird", mask_paird.shape)
            mask_paird = mask_paird.view(B * K, 2).to(device)
            # [:, 0], [:, 1]のどちらかがTrueならTrue
            mask_paird = mask_paird[:, 0] | mask_paird[:, 1]
            # print("mask_paird", mask_paird.shape)
            edge_weight = edge_weight*~mask_paird.unsqueeze(-1)
            # print("edge_weight", edge_weight.shape)
            # pass
        big_out = self.gnn(
            key_value_input_flatten,
            self.edge_list.t().to(device),
            edge_weight=edge_weight
        )

        # 出力投影 + residual dropout
        big_out = self.resid_drop(self.proj(big_out))  # [sum_of_all_nodes, C]

        # 形を (N, T, C), (M, T, C), (N+M, T, C) に戻す
        if self.att_type == 'self':
            # x のみ
            # sub_size = N*T
            big_out = big_out.view(B, N, T, C)
            ...
            if c is not None:
                # shape: (M, T, C)
                big_out = big_out.view(B, M, T, C)
                # もともとのコードでは 平均プーリングをしている例があったので合わせる
            else:
                big_out = big_out.view(B, N, T, C)
            big_out = big_out.mean(dim=1, keepdim=True)

        else:  # 'hybrid'
            # sub_size = (N+M)*T
            # 必要に応じて x部分とc部分を切り出すなどの設計も可能
            total_n = N + (M if c is not None else 0)
            big_out = big_out.view(B, total_n, T, C)
...
class ParticleAttention(nn.Module):
    ...
    def forward(self, x, c=None, mask=None, return_attention=False):
        ...
        # print("key_value_input", key_value_input.shape)
...
class EITActor(BasePolicy):
    ...
    def forward(self, obs, return_attention=False):
        ...
        # print("x_agg0", x_agg.shape)

        x_agg = x_agg.squeeze(1, 2)  # [bs, embed_dim]
        # print("x_agg1", x_agg.shape)

        # final layer norm
        x_agg = self.linear_out(self.ln(x_agg))
        # print("x_agg2", x_agg.shape)
